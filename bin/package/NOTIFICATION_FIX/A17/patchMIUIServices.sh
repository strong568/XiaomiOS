...
jar_util() 
{
    ...
    #binary
    if [[ $3 == "fw" ]]; then 
        bak="java -jar $work_dir/bin/apktool/baksmaliv2.jar d --api 37"
        sma="java -jar $work_dir/bin/apktool/smaliv2.jar a --api 37"
    fi
    ...
    if [[ $1 == "d" ]]; then
        ...
        if [[ -f $work_dir/build/baserom/images/system_ext/framework/miui-services.jar ]]; then
            ...
            if [[ -d $work_dir/jar_temp/"$2.out" ]]; then
                ...
                # cp $work_dir/bin/shPlugin/NOTIFICATION_FIX/A17/xBuild.smali $work_dir/jar_temp/$2.out/classes.dex.out/miuix/os/
            fi
        fi
    else 
        if [[ $1 == "a" ]]; then 
